...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await getAdminClient().from('users').update(snakify<Record<string,unknown>>(body)).eq('id', id).eq('organization_id', user.organizationId).select('*, user_profiles(first_name,last_name)').single()
